...
static void ExpectAllElementsIn(
    const std::vector<expr::Value>& sample,
...
TEST_F(AggregateExecTest, RandomSampleBasicTest) {
  std::cerr << "RandomSampleBasicTest\n";
...
TEST_F(AggregateExecTest, RandomSampleNilHandlingTest) {
  std::cerr << "RandomSampleNilHandlingTest\n";
...
TEST_F(AggregateExecTest, RandomSampleTypeHandlingTest) {
  std::cerr << "RandomSampleTypeHandlingTest\n";
...
TEST_F(AggregateExecTest, RandomSampleMultipleReducersTest) {
  std::cerr << "RandomSampleMultipleReducersTest\n";
...
TEST_F(AggregateExecTest, RandomSampleGroupByTest) {
  std::cerr << "RandomSampleGroupByTest\n";
...
// Feature: random-sample-reducer-vector-refactor, Property 1: GetResult always returns a vector Value
...
// Feature: random-sample-reducer-vector-refactor, Property 3: Output vector size is bounded by min(sample_size, non-nil count)
...
TEST_F(AggregateExecTest, RandomSampleProperty3SizeBound) {
  ...
  // MakeData creates all non-nil numeric values, so non-nil count == input_count.
...
// Feature: random-sample-reducer-vector-refactor, Property 4: All valid sample sizes produce no error
...
TEST_F(AggregateExecTest, RandomSampleProperty4AllValidSizes) {
  ...
  for (int k : sizes) {
    ...
    EXPECT_TRUE(status.ok())
        << "Expected ok status for sample_size=" << k
        << " but got: " << status;
  }
}

// Feature: random-sample-reducer-vector-refactor, Property 2: Output vector preserves non-nil element types
  ...
  // Test with mixed types and nils — no nil should appear in output
  {
    ...
    for (int i = 0; i < 9; ++i) {
      ...
      if (i % 3 == 0) {
        ...
        rec->fields_[0] =
            expr::Value(std::string("str") + std::to_string(i));
      }
      rec->fields_[1] = expr::Value(1.0);
      records.emplace_back(std::move(rec));
    }
