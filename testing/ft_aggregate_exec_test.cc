...
TEST_F(AggregateExecTest, FirstValueReducerEdgeCasesTest) {
  ...
  {
    std::cerr << "Edge Case Test 3: Nil return value preserved with valid "
                 "comparison\n";
    auto param = MakeStages(
  ...
  {
    std::cerr
        << "Edge Case Test 5: String comparison with lexicographic ordering\n";
...
TEST_F(AggregateExecTest, FirstValueSimpleModePropertyTest) {
  ...
  for (size_t group_size = 1; group_size <= 20; ++group_size) {
    test_cases.push_back(
        {group_size, 0.0, "Group size " + std::to_string(group_size)});
  }

  for (size_t group_size : {25, 50, 75, 100}) {
    test_cases.push_back(
        {group_size, 0.0, "Large group size " + std::to_string(group_size)});
  }
...
TEST_F(AggregateExecTest, FirstValueSortedAscPropertyTest) {
  ...
  for (size_t min_pos : {0, 1, 2, 3, 4}) {
    ...
    test_cases.push_back(
        {n1_values, min_val, "Min at position " + std::to_string(min_pos)});
  }

  test_cases.push_back({{50.0, -100.0, 0.0, 25.0}, -100.0, "Negative minimum"});

  test_cases.push_back(
      {{1.001, 1.0, 1.002, 1.003}, 1.0, "Close values precision"});

  test_cases.push_back(
      {{1000000.0, -1000000.0, 500000.0, 0.0}, -1000000.0, "Large range"});
  ...
  for (size_t iter = 0; iter < 20; ++iter) {
    for (size_t pattern = 0; pattern < 4; ++pattern) {
      ...
      for (size_t i = 0; i < group_size; ++i) {
        if ((pattern == 0 && i == 0) || (pattern == 1 && i == group_size - 1) ||
            (pattern == 2 && i == group_size / 2) || (pattern == 3 && i == 1)) {
          n1_values.push_back(min_val);
        } else {
      ...
      test_cases.push_back({n1_values, min_val,
                            "Iter " + std::to_string(iter) + " Pattern " +
  ...
  for (const auto &tc : test_cases) {
    ...
    // Test with explicit ASC (nargs=4)
    {
      ...
      for (double n1_val : tc.n1_values) {
        auto rec = std::make_unique<Record>(2);
        rec->fields_[0] =
            expr::Value(n1_val);  // n1 = value to return and compare
        rec->fields_[1] =
            expr::Value(double(group_key));  // n2 = constant for grouping
        test_records.emplace_back(std::move(rec));
      }
      ...
      EXPECT_NEAR(result, tc.expected_min, 0.001)
          << "Property violated (ASC): Expected " << tc.expected_min << ", got "
          << result << " in " << tc.description;
    }
...
// with BY and DESC, the result should equal the return property value from the
// record with the largest comparison property value.
TEST_F(AggregateExecTest, FirstValueSortedDescPropertyTest) {
  // the maximum comparison property across various group sizes and value
  // ranges.
  //

  struct PropertyTestCase {
    std::vector<double>
        n1_values;        // Values for n1 field (return and comparison)
  ...
  for (size_t max_pos : {0, 1, 2, 3, 4}) {
    ...
    test_cases.push_back(
        {n1_values, max_val, "Max at position " + std::to_string(max_pos)});
  }

  test_cases.push_back(
      {{-50.0, -100.0, -10.0, -25.0}, -10.0, "Negative maximum"});

  test_cases.push_back(
      {{-50.0, 100.0, 0.0, -25.0}, 100.0, "Mixed pos/neg maximum"});

  test_cases.push_back(
      {{1.001, 1.0, 1.002, 1.003}, 1.003, "Close values precision"});
  ...
  test_cases.push_back(
      {{100.0, 200.0, 50.0, 200.0}, 200.0, "Multiple maximums"});
  ...
  for (const auto &tc : test_cases) {
    ...
    EXPECT_NEAR(result, tc.expected_max, 0.001)
        << "Property violated (DESC): Expected " << tc.expected_max << ", got "
        << result << " in " << tc.description;
  }
...
// when FIRST_VALUE is used with BY clause, records with nil comparison values
// should be excluded from consideration, and the result should be determined
// only from records with non-nil comparison values.
TEST_F(AggregateExecTest, FirstValueNilComparisonSkippingPropertyTest) {
  // the optimal value. We test with 100+ iterations using groups with mixed nil
  // and non-nil values.
  //

  struct PropertyTestCase {
    std::vector<std::optional<double>>
        n1_values;  // Values for n1 (nil if nullopt)
    std::optional<double>
        expected_result_asc;  // Expected result for ASC (nullopt if all nil)
    std::optional<double>
        expected_result_desc;  // Expected result for DESC (nullopt if all nil)
    std::string description;
  };
  ...
  test_cases.push_back({{std::nullopt, 50.0, 75.0, 100.0, std::nullopt},
                        50.0,
                        100.0,
                        "Nils at both ends"});
  ...
  test_cases.push_back({{std::nullopt, 30.0, std::nullopt, std::nullopt, 70.0},
                        30.0,
                        70.0,
                        "Two non-nil values"});
  ...
  for (size_t group_size = 3; group_size <= 15; ++group_size) {
    ...
    for (size_t i = 0; i < group_size; ++i) {
      ...
      if (std::rand() % 10 == 0) {
        ...
        double val = static_cast<double>(std::rand() % 1000 -
                                         500);  // Range: -500 to 499
  ...
  for (const auto &tc : test_cases) {
    ...
    // Test with ASC mode
    {
      ...
      if (tc.expected_result_asc.has_value()) {
        ...
        EXPECT_TRUE(record->fields_.at(2).IsNil())
            << "Property violated (ASC): Expected nil when all comparison "
               "values are nil in "
            << tc.description;
      }
    }
    ...
    // Test with DESC mode
    {
      ...
      if (tc.expected_result_desc.has_value()) {
        ...
        EXPECT_TRUE(record->fields_.at(2).IsNil())
            << "Property violated (DESC): Expected nil when all comparison "
               "values are nil in "
            << tc.description;
      }
    }
...
// the result should be identical to using 4 arguments with ASC explicitly
// specified.
TEST_F(AggregateExecTest, FirstValueDefaultOrderPropertyTest) {
  // identically to the 4-argument form with explicit ASC across various group
  ...
  test_cases.push_back(
      {{-100.0, -50.0, -10.0, -25.0}, -100.0, "Negative minimum"});

  test_cases.push_back(
      {{100.0, -50.0, 0.0, 25.0}, -50.0, "Mixed pos/neg minimum"});

  test_cases.push_back(
      {{1.003, 1.001, 1.002, 1.0}, 1.0, "Close values precision"});
  ...
  for (const auto &tc : test_cases) {
    ...
    double result_3arg;
    {
      ...
      ASSERT_EQ(test_records.size(), 1)
          << "Expected one grouped record (3-arg)";

      auto record = test_records.pop_front();
      ASSERT_TRUE(record->fields_.at(2).IsDouble())
          << "Result not a double (3-arg)";

      result_3arg = *(record->fields_.at(2).AsDouble());
      EXPECT_NEAR(result_3arg, tc.expected_result, 0.001)
          << "3-arg form: Expected " << tc.expected_result << ", got "
          << result_3arg << " in " << tc.description;
    }
    ...
    double result_4arg;
    {
      ...
      ASSERT_EQ(test_records.size(), 1)
          << "Expected one grouped record (4-arg)";

      auto record = test_records.pop_front();
      ASSERT_TRUE(record->fields_.at(2).IsDouble())
          << "Result not a double (4-arg)";

      result_4arg = *(record->fields_.at(2).AsDouble());
      EXPECT_NEAR(result_4arg, tc.expected_result, 0.001)
          << "4-arg form: Expected " << tc.expected_result << ", got "
          << result_4arg << " in " << tc.description;
    }
    ...
    EXPECT_NEAR(result_3arg, result_4arg, 0.001)
        ...
        << ") differs from 4-arg ASC form result (" << result_4arg << ") in "
        << tc.description;
  }
...
// (minimum for ASC, maximum for DESC), the result should be the return property
// value from the first record encountered with that optimal comparison value.
TEST_F(AggregateExecTest, FirstValueTieBreakingPropertyTest) {
  // the first encountered record's value is returned. We test with 100+
  // iterations using groups with duplicate optimal values at different
  // positions.
  //
  ...
  struct PropertyTestCase {
    ...
    double expected_result_asc;   // Expected return value for ASC (first with
                                  // min comparison)
    double expected_result_desc;  // Expected return value for DESC (first with
                                  // max comparison)
    std::string description;
  };
  ...
  test_cases.push_back(
      {{111.0, 222.0, 333.0, 444.0, 555.0},  // return values
       {5.0, 100.0, 5.0, 200.0,
        5.0},  // comparison values (min=5.0 at pos 0, 2, 4)
  ...
  for (size_t group_size = 2; group_size <= 10; ++group_size) {
    ...
    test_cases.push_back(
        {return_values, comparison_values,
  ...
  test_cases.push_back(
      {{-100.0, -200.0, -300.0, -400.0},  // return values (all negative)
       {-50.0, 100.0, -50.0,
        200.0},  // comparison values (min=-50.0 at pos 0 and 2)
       -100.0,   // ASC: first record with min (pos 0, return -100.0)
       -400.0,   // DESC: first record with max (pos 3, return -400.0)
  ...
  for (size_t iter = 0; iter < 25; ++iter) {
    for (size_t pattern = 0; pattern < 4; ++pattern) {
      ...
      if (pattern == 0) {
        tie_pos1 = 0;
        tie_pos2 = 1;  // Tie at start
      } else if (pattern == 1) {
        tie_pos1 = group_size - 2;
        tie_pos2 = group_size - 1;  // Tie at end
      } else if (pattern == 2) {
        tie_pos1 = 0;
        tie_pos2 = group_size - 1;  // Tie at extremes
      } else {
        tie_pos1 = 1;
        tie_pos2 = 3;  // Tie in middle
      }
      ...
      test_cases.push_back({return_values, comparison_values,
                            first_return_with_min, first_return_with_max,
                            "Iter " + std::to_string(iter) + " Pattern " +
                                std::to_string(pattern)});
    }
  }
  ...
  for (const auto &tc : test_cases) {
    ...
    ASSERT_EQ(tc.return_values.size(), tc.comparison_values.size())
        << "Test case setup error: return and comparison vectors must have "
           "same size";
    ...
    // Test with ASC mode
    {
      ...
      for (size_t i = 0; i < tc.comparison_values.size(); ++i) {
        auto rec = std::make_unique<Record>(2);
        rec->fields_[0] =
            expr::Value(tc.comparison_values[i]);  // n1 = comparison value (and
                                                   // return value)
      ...
      auto record = test_records.pop_front();
      ASSERT_TRUE(record->fields_.at(2).IsDouble())
          << "Result not a double (ASC)";

      double result = *(record->fields_.at(2).AsDouble());

      double expected_min = *std::min_element(tc.comparison_values.begin(),
                                              tc.comparison_values.end());

      EXPECT_NEAR(result, expected_min, 0.001)
          << "Property violated (ASC): Expected " << expected_min
          << " (minimum comparison value), got " << result << " in "
          << tc.description;
    }
    ...
    // Test with DESC mode
    {
      ...
      auto record = test_records.pop_front();
      ASSERT_TRUE(record->fields_.at(2).IsDouble())
          << "Result not a double (DESC)";

      double result = *(record->fields_.at(2).AsDouble());

      double expected_max = *std::max_element(tc.comparison_values.begin(),
                                              tc.comparison_values.end());

      EXPECT_NEAR(result, expected_max, 0.001)
          << "Property violated (DESC): Expected " << expected_max
          << " (maximum comparison value), got " << result << " in "
          << tc.description;
    }
  }
...
TEST_F(AggregateExecTest, FirstValueNilReturnPreservationPropertyTest) {
  // with nil return property has the optimal comparison value. We test with
  // 100+ iterations using groups where the optimal record has a nil return
  // value.
  //
  ...
  for (const auto &tc : test_cases) {
    ...
    ASSERT_EQ(tc.return_values.size(), tc.comparison_values.size())
        << "Test case setup error: return and comparison value counts must "
           "match";

    // Test with the specified order
    std::string order = tc.is_desc ? "'\"DESC\"'" : "'\"ASC\"'";
    std::string query =
        "groupby 1 @n2 reduce first_value 4 @n1 '\"BY\"' @n3 " + order;
    ...
    if (tc.expected_result.has_value()) {
      ASSERT_TRUE(record->fields_.at(result_field_index).IsDouble())
          << "Result should be double when optimal record has non-nil return "
             "value"
          << " in " << tc.description;
      double result = *(record->fields_.at(result_field_index).AsDouble());
      EXPECT_NEAR(result, *tc.expected_result, 0.001)
          << "Property violated: Expected " << *tc.expected_result << ", got "
          << result << " in " << tc.description;
    } else {
      EXPECT_TRUE(record->fields_.at(result_field_index).IsNil())
          << "Property violated: Expected nil when optimal record has nil "
             "return value in "
          << tc.description;
    }
  }
...
// produce correct ordering for both numeric values (numerical comparison) and
// string values (lexicographic comparison).
TEST_F(AggregateExecTest, FirstValueTypeCompatibilityPropertyTest) {
  // with appropriate comparison semantics. We test with 100+ iterations using
  // groups with both numeric and string comparison values.
  //
  ...
  for (size_t i = 0; i < 50; ++i) {
    ...
    test_cases.push_back({"Numeric comparison, group size " +
                              std::to_string(group_size) +
                              (is_desc ? " DESC" : " ASC"),
                          numeric_vals,
                          {},    // empty string values
  ...
  for (size_t i = 0; i < 50; ++i) {
    ...
    test_cases.push_back({"String comparison, group size " +
                              std::to_string(group_size) +
                              (is_desc ? " DESC" : " ASC"),
  ...
  // Negative numbers
  test_cases.push_back({"Numeric: negative numbers ASC",
                        {-100.0, -50.0, -200.0, -10.0},
                        {},
                        true,
                        false});

  test_cases.push_back({"Numeric: negative numbers DESC",
                        {-100.0, -50.0, -200.0, -10.0},
                        {},
                        true,
                        true});
  ...
  test_cases.push_back({"Numeric: mixed positive/negative ASC",
                        {100.0, -50.0, 0.0, -100.0, 50.0},
                        {},
                        true,
                        false});

  test_cases.push_back({"Numeric: mixed positive/negative DESC",
                        {100.0, -50.0, 0.0, -100.0, 50.0},
                        {},
                        true,
                        true});
  ...
  test_cases.push_back({"Numeric: small differences ASC",
                        {1.001, 1.002, 1.000, 1.003},
                        {},
                        true,
                        false});

  test_cases.push_back({"Numeric: small differences DESC",
                        {1.001, 1.002, 1.000, 1.003},
                        {},
                        true,
                        true});
  ...
  test_cases.push_back({"String: lexicographic ordering ASC (10 < 2)",
                        {},
                        {"10", "2", "20", "3"},
                        false,
                        false});

  test_cases.push_back({"String: lexicographic ordering DESC (3 > 20)",
                        {},
                        {"10", "2", "20", "3"},
                        false,
                        true});
  ...
  // Case sensitivity
  test_cases.push_back({"String: case sensitivity ASC",
                        {},
                        {"apple", "Apple", "APPLE", "aPpLe"},
                        false,
                        false});

  test_cases.push_back({"String: case sensitivity DESC",
                        {},
                        {"apple", "Apple", "APPLE", "aPpLe"},
                        false,
                        true});
  ...
  // Prefixes
  test_cases.push_back({"String: prefix ordering ASC",
                        {},
                        {"abc", "ab", "abcd", "a"},
                        false,
                        false});

  test_cases.push_back({"String: prefix ordering DESC",
                        {},
                        {"abc", "ab", "abcd", "a"},
                        false,
                        true});
  ...
  // Empty strings
  test_cases.push_back({"String: with empty string ASC",
                        {},
                        {"", "a", "aa", "aaa"},
                        false,
                        false});

  test_cases.push_back({"String: with empty string DESC",
                        {},
                        {"", "a", "aa", "aaa"},
                        false,
                        true});
  ...
  // Special characters
  test_cases.push_back({"String: special characters ASC",
                        {},
                        {"abc", "a-c", "a_c", "a c"},
                        false,
                        false});

  test_cases.push_back({"String: special characters DESC",
                        {},
                        {"abc", "a-c", "a_c", "a c"},
                        false,
                        true});
  ...
  for (const auto &tc : test_cases) {
    ...
    if (tc.is_numeric) {
      ...
      ASSERT_FALSE(tc.numeric_values.empty())
          << "Test case setup error: numeric values should not be empty for "
             "numeric test";

      std::string order = tc.is_desc ? "'\"DESC\"'" : "'\"ASC\"'";
      std::string query =
          "groupby 1 @n2 reduce first_value 4 @n1 '\"BY\"' @n1 " + order;
      ...
      for (size_t i = 0; i < tc.numeric_values.size(); ++i) {
        auto rec = std::make_unique<Record>(2);
        rec->fields_[0] =
            expr::Value(tc.numeric_values[i]);     // n1 = numeric value
        rec->fields_[1] = expr::Value(group_key);  // n2 = constant for grouping
        test_records.emplace_back(std::move(rec));
      }
      ...
      ASSERT_EQ(test_records.size(), 1)
          << "Expected one grouped record (numeric)";

      auto record = test_records.pop_front();

      // Result is at field index 2 when using 2 input fields (n1, n2)
      // Field layout: [0: Nil (n1 not in output), 1: group key (@n2), 2:
      // reducer result]
      size_t result_field_index = 2;

      ASSERT_TRUE(record->fields_.at(result_field_index).IsDouble())
          << "Result not a double (numeric)";

      double result = *(record->fields_.at(result_field_index).AsDouble());

      double expected;
      if (tc.is_desc) {
        expected = *std::max_element(tc.numeric_values.begin(),
                                     tc.numeric_values.end());
      } else {
        expected = *std::min_element(tc.numeric_values.begin(),
                                     tc.numeric_values.end());
      }
      ...
      ASSERT_FALSE(tc.string_values.empty())
          << "Test case setup error: string values should not be empty for "
             "string test";

      // For string comparison, we need to use the actual string values
      // Since the test infrastructure uses numeric fields, we'll encode strings
      // as their lexicographic order and verify the comparison works correctly
      ...
      if (tc.is_desc) {
        ...
        expected_str =
            sorted_strings.front();  // First in sorted order (minimum)
      }

      auto it = std::find(tc.string_values.begin(), tc.string_values.end(),
                          expected_str);
      ASSERT_NE(it, tc.string_values.end())
          << "Expected string not found in original array";
      ...
      for (size_t i = 0; i < tc.string_values.size(); ++i) {
        auto rec = std::make_unique<Record>(2);
        rec->fields_[0] =
            expr::Value(tc.string_values[i]);      // n1 = string value
        rec->fields_[1] = expr::Value(group_key);  // n2 = constant for grouping
        test_records.emplace_back(std::move(rec));
      }
      ...
      ASSERT_EQ(test_records.size(), 1)
          << "Expected one grouped record (string)";
      ...
      std::string result =
          std::string(record->fields_.at(result_field_index).AsStringView());
