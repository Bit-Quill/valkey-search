...
// --- SetQueryVector / GetQueryVector round-trip ---

TEST_F(VectorRangePredicateTest, QueryVectorRoundTrip) {
  VectorRangePredicate pred("vec", "v_id", 1.0, "blob", std::nullopt,
                            std::nullopt);

  EXPECT_TRUE(pred.GetQueryVector().empty());

  std::string query_blob(16, '\0');
  for (int i = 0; i < 16; ++i) {
    query_blob[i] = static_cast<char>(i);
  }
  pred.SetQueryVector(query_blob);

  EXPECT_EQ(pred.GetQueryVector(), query_blob);
  EXPECT_EQ(pred.GetQueryVector().size(), 16);
}

TEST_F(VectorRangePredicateTest, QueryVectorOverwrite) {
  VectorRangePredicate pred("vec", "v_id", 1.0, "blob", std::nullopt,
                            std::nullopt);

  pred.SetQueryVector("first_query");
  EXPECT_EQ(pred.GetQueryVector(), "first_query");

  pred.SetQueryVector("second_query");
  EXPECT_EQ(pred.GetQueryVector(), "second_query");
}

TEST_F(VectorRangePredicateTest, QueryVectorBinaryData) {
  VectorRangePredicate pred("vec", "v_id", 1.0, "blob", std::nullopt,
                            std::nullopt);
  ...
  pred.SetQueryVector(binary_blob);
  EXPECT_EQ(pred.GetQueryVector().size(), 16);
  EXPECT_EQ(pred.GetQueryVector(), binary_blob);
}
