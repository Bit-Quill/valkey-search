...
| FIRST_VALUE 1 <expression>                                | The first value encountered in the group. Order depends on record retrieval order and is non-deterministic.                        |
| FIRST_VALUE 3 <expression> BY <expression>                | The value with the minimum comparison property (ascending order). When multiple records have equal comparison values, returns the first encountered. |
| FIRST_VALUE 4 <expression> BY <expression> ASC&#124;DESC  | The value with the minimum (ASC) or maximum (DESC) comparison property. When multiple records have equal comparison values, returns the first encountered. |
